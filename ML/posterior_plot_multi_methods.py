...
for i, method in enumerate(methods):
   ...
   hist2d(xHI_mean_post, logfX_post, levels=[1-np.exp(-2.)], smooth=True, plot_datapoints=False, 
                 plot_density=False, plot_contours=True, fill_contours=True, color=colours[i], lighten_fill=False,
                   contourf_kwargs={'zorder': zorder}, sec_colour=sec_colours[i], contour_kwargs={'zorder': zorder} )
   ...
   rectangle = Rectangle((0, 0), 20, 18,
                              linewidth=1,  facecolor=sec_colours[i], edgecolor=colours[i], label=f'{method_labels[i]}')#, G={g_score:.2f}')
